...
3. **Delegate**: one implementation subagent per issue, in an isolated worktree, model per the issue's `model:` label (sonnet → sonnet, opus → opus). `model:` labels are minimums, not ceilings — if a stronger model is available in-session, prefer it for `model:opus` issues; never downgrade below the label. The subagent's instructions: assign yourself, follow the `work-issue` skill, end with a PR. Give it the issue number, its assigned ports (see Worktrees & ports), and nothing else — the board carries the context.
...
5. **Merge** when: the latest reviewer verdict is `VERDICT: approve` + CI green. (Until #9 lands there is no CI — rely on the verdict plus the checks reported in the PR body.) Issues labelled `human-verify` additionally require the user's explicit go-ahead — see Manual verification checkpoints below. Merge with `gh pr merge NN --squash --admin`: the main ruleset requires PRs and approvals, but this is a single-account project, so admin bypass is the sanctioned merge path — **only after** the verdict/CI/checkpoint gates above have passed; the bypass skips GitHub's checks, never ours. `Closes #NN` closes the issue and native dependencies update the frontier automatically.
